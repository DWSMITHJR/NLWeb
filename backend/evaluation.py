...
class RetrievalMetrics:
    """A collection of static methods for calculating retrieval metrics."""
    ...
    @staticmethod
    def calculate_mrr(
        retrieved: List[DocumentChunk], relevant: List[DocumentChunk]
    ) -> float:
        """
...
class AnswerQualityMetrics:
    """A collection of static methods for calculating answer quality metrics."""

    @staticmethod
    def calculate_rouge(
        generated: str, reference: str, n_gram: int = 1
    ) -> Dict[str, float]:
        """
        ...
        return {f"rouge_{n_gram}": {"precision": precision, "recall": recall, "f1": f1}}
    ...
    def calculate_bleu(
        ...
        brevity_penalty = (
            min(1.0, np.exp(1 - ref_len / gen_len)) if gen_len > 0 else 0.0
        )
        ...
        if min(precisions) == 0:
            ...
            bleu = brevity_penalty * np.exp(
                sum(w * np.log(p) for w, p in zip(weights, precisions) if p > 0)
            )
...
class EvaluationResult:
    """
    Stores and aggregates evaluation results for a single configuration.

    Attributes:
        config: The configuration that was evaluated.
        metrics: A dictionary to store retrieval and answer quality metrics.
    """

    def __init__(self, config: Dict[str, Any]):
        """Initializes the EvaluationResult with the configuration used for the test."""
        self.config = config
        self.metrics = {"retrieval": [], "answer_quality": []}

    def add_retrieval_metrics(self, metrics: Dict[str, float]):
        """Adds a set of retrieval metrics to the result."""
        self.metrics["retrieval"].append(metrics)

    def add_answer_quality_metrics(self, metrics: Dict[str, float]):
        """Adds a set of answer quality metrics to the result."""
        self.metrics["answer_quality"].append(metrics)

    def get_summary(self) -> Dict[str, Any]:
        """Calculates and returns a summary of all collected metrics."""
        summary = {
            "config": self.config,
            "retrieval_metrics": {},
        ...
        if self.metrics["answer_quality"]:
            ...
            for metric in self.metrics["answer_quality"][0].keys():
                values = [
                    m[metric] for m in self.metrics["answer_quality"] if metric in m
                ]
                if values:
