...
from ..models import Document, DocumentProcessorConfig, ChunkingStrategy
from ..document_processor import DocumentProcessor
from ..prompt_templates import PromptTemplateManager, TemplateType
from .retrievers.base import BaseRetriever
from .retrievers.faiss_retriever import FAISSRetriever
from ..evaluation import RetrievalMetrics, AnswerQualityMetrics, EvaluationResult
...
class AutoMLOrchestrator:
    ...
    def __init__(
        ...
        """
        Initialize the AutoML orchestrator
    ...
    def _create_retriever(self, config: Dict[str, Any]) -> BaseRetriever:
        ...
        if retriever_type == 'faiss':
            ...
            return HybridRetriever(
                ...
                faiss_model_name=config.get('embedding_model', 'sentence-transformers/all-MiniLM-L6-v2'),
                normalize_embeddings=config.get('normalize_embeddings', True)
            )
        else:
            raise ValueError(f"Unsupported retriever type: {retriever_type}")
    
    def _create_processor_config(self, config: Dict[str, Any]) -> DocumentProcessorConfig:
        """Create a document processor configuration"""
        return DocumentProcessorConfig(
            chunk_size=config.get('chunk_size', 256),
            chunk_overlap=config.get('chunk_overlap', 50),
            chunking_strategy=ChunkingStrategy(config.get('chunking_strategy', 'fixed'))
        )
    ...
    def _evaluate_retrieval(
        ...
        Args:
            ...
            test_queries: List of test queries with relevant documents
        ...
        for query_data in test_queries:
            ...
            metrics.append({
                'query': query,
                **RetrievalMetrics.calculate_precision_recall(retrieved_chunks, relevant_chunks, k=top_k),
                'mrr': RetrievalMetrics.calculate_mrr(retrieved_chunks, relevant_chunks)
            })
    ...
    def _evaluate_answer_quality(
        ...
        """
        Evaluate answer quality using the given prompt template
        
        Args:
            retriever: The retriever to use
            test_queries: List of test queries with reference answers
    ...
    def _evaluate_configuration(
        ...
        Args:
            ...
            train_documents: Documents to train the retriever
            test_queries: Test queries with relevant documents and answers
        ...
        try:
            ...
            try:
                ...
                config['prompt_template'] = template.name  # Store template name in config
            except ValueError:
        ...
        for _ in range(num_configs):
            ...
            for param, values in search_space.items():
                if param not in config or param in [
                    ...
                    'bm25_weight', 'faiss_weight', 'prompt_template'
                ]:
            ...
            if 'prompt_template' in config and config['prompt_template'] not in [t.value for t in available_templates]:
                config['prompt_template'] = random.choice([t.value for t in available_templates])
    ...
    def run(
        ...
        Args:
            train_documents: Documents to train the retriever
            test_queries: Test queries with relevant documents
            base_config: Base configuration
            num_configs: Number of configurations to test
            save_every: Save results every N configurations
    ...
    def get_best_config(self) -> Tuple[Dict[str, Any], float]:
        """
