...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("automl_test.log")],
)
...
def test_automl_integration():
    """Tests the full AutoML system integration."""
    try:
        ...
        from backend.prompt_templates import TemplateType
        ...
        documents = [
            Document(
                id=doc["id"], content=doc["content"], metadata=doc.get("metadata", {})
            )
            for doc in raw_docs
        ]
        ...
        assert results is not None, "AutoML run should produce results"
        assert "best_config" in results, "Results should contain a 'best_config' key"
        assert "best_score" in results, "Results should contain a 'best_score' key"
        assert results["best_score"] > -float("inf"), "Best score should be updated"
        assert (
            len(results["results"]) > 0
        ), "At least one configuration should be tested"

    except Exception as e:
        logger.error(f"Error in AutoML test: {str(e)}", exc_info=True)
        assert False, f"AutoML integration test failed with an exception: {e}"
