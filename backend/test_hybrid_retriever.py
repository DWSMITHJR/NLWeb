...
def evaluate_retriever(retriever, test_queries: List[Dict[str, Any]], top_k: int = 3) -> Dict[str, float]:
...
def test_hybrid_retriever():
    ...
    for bm25_w, faiss_w in weight_combinations:
        ...
        results[f'hybrid_{int(bm25_w*100)}_{int(faiss_w*100)}'] = evaluate_retriever(hybrid, test_queries)
    ...
    for name, metrics in results.items():
        print(f"{name:<20} {metrics['precision']:.4f}    {metrics['recall']:.4f}    {metrics['f1']:.4f}    {metrics['mrr']:.4f}")
